...
func LoadChains(path string) {
	...
	if err != nil {
		useDefaultChains()
		log.Printf("[chains] using built-in defaults (%d chains)", len(Chains))
		return
	}
	var list []ChainConfig
	if err := json.Unmarshal(data, &list); err != nil {
		log.Printf("[chains] WARNING: parse %s failed: %v — using defaults", path, err)
		useDefaultChains()
		return
	}
	for _, c := range list {
	...
	log.Printf("[chains] loaded %d chains from %s", len(Chains), path)
}
