...
func NewWalletHandler(db *gorm.DB, sdkClient *sdk.Client, baseURL string) *WalletHandler {
	return &WalletHandler{db: db, sdk: sdkClient, baseURL: baseURL}
}
...
func (h *WalletHandler) CreateWallet(c *gin.Context) {
	...
	if genErr != nil || !keyResult.Success {
		...
		c.JSON(http.StatusBadGateway, gin.H{"error": msg, "wallet_id": wallet.ID})
		return
	}
	...
	if addrErr != nil {
		h.db.Model(&wallet).Updates(map[string]interface{}{"status": "error"})
		c.JSON(http.StatusInternalServerError, gin.H{"error": "address derivation failed: " + addrErr.Error()})
		return
	}
...
func (h *WalletHandler) DeleteWallet(c *gin.Context) {
	...
	if h.sdk != nil && wallet.KeyName != "" {
		if _, err := h.sdk.DeletePublicKey(c.Request.Context(), wallet.KeyName); err != nil {
			log.Printf("[wallet] DeletePublicKey failed wallet_id=%d key=%s err=%v", wallet.ID, wallet.KeyName, err)
		}
	}
...
func (h *WalletHandler) Sign(c *gin.Context) {
	...
	if signErr != nil {
		c.JSON(http.StatusBadGateway, gin.H{"error": signErr.Error()})
		return
	}
	if !result.Success {
...
func (h *WalletHandler) SetPolicy(c *gin.Context) {
	...
	if !ok3 || t.Sign() <= 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "threshold_amount must be a positive number"})
		return
	}
	if req.DailyLimit != "" {
		dl, ok4 := new(big.Float).SetString(req.DailyLimit)
		if !ok4 || dl.Sign() <= 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "daily_limit must be a positive number"})
			return
		}
	}
	...
	if !isPasskeyAuth(c) {
		...
		approval, created := createPendingApproval(h.db, c, wallet.ID, "policy_change", proposed)
		if !created {
			return
		}
	...
	if err := h.db.Save(&policy).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "save policy failed"})
		return
	}
...
func (h *WalletHandler) createApprovalRequest(c *gin.Context, wallet model.Wallet, req SignRequest, msgBytes []byte, policy *model.ApprovalPolicy, txParams string) {
	...
	if jsonErr != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "marshal tx_context failed"})
		return
	}
...
func (h *WalletHandler) Transfer(c *gin.Context) {
	...
	case "evm":
		...
		if addrErr != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "to: " + addrErr.Error()})
			return
		}
		req.To = toAddr
		...
		if req.Token != nil {
			...
			if err := h.db.Where("wallet_id = ? AND contract_address = ?", wallet.ID, tokenContractAddr).First(&allowed).Error; err != nil {
				c.JSON(http.StatusForbidden, gin.H{"error": "contract not whitelisted: " + tokenContractAddr})
				return
			}
			...
			if err != nil {
				log.Printf("[transfer] build ERC-20 tx failed wallet_id=%d chain=%s rpcURL=%s err=%v", wallet.ID, wallet.Chain, rpcURL, err)
				c.JSON(http.StatusBadGateway, gin.H{"error": "build contract tx: " + err.Error()})
				return
			}
			...
			if err != nil {
				log.Printf("[transfer] build ETH tx failed wallet_id=%d chain=%s rpcURL=%s err=%v", wallet.ID, wallet.Chain, rpcURL, err)
				c.JSON(http.StatusBadGateway, gin.H{"error": "build tx: " + err.Error()})
				return
			}
			signingMsg = txData.SigningHash
			b, marshalErr := json.Marshal(txData.Params)
	...
	// Check daily spend limit (hard block — no approval path for this).
	if policyFound && policy.DailyLimit != "" {
		if exceeded, msg := dailyLimitExceeded(&policy, req.Amount); exceeded {
	...
	if err != nil || !result.Success {
		...
		log.Printf("[transfer] TEE signing failed wallet_id=%d key=%s err=%s", wallet.ID, wallet.KeyName, errMsg)
	...
	// Update daily spent counter after successful broadcast.
	if policyFound && policy.DailyLimit != "" {
		addDailySpent(h.db, &policy, req.Amount)
	}
...
func broadcastSigned(wallet model.Wallet, txParamsJSON string, sig []byte) (string, error) {
	...
	case "evm":
		...
		return chain.AssembleAndBroadcastETH(cfg.RPCURL, params, sig, wallet.Address)
	case "solana":
...
func respondBroadcastError(c *gin.Context, err error) {
	...
	if strings.Contains(msg, "rpc error:") {
		c.JSON(http.StatusBadRequest, gin.H{"error": "transaction rejected: " + msg})
		return
	}
	c.JSON(http.StatusBadGateway, gin.H{"error": "broadcast failed: " + msg})
}

// dailyLimitExceeded checks whether adding `amount` would exceed the policy's daily limit.
// It resets the counter automatically when a new UTC day is detected.
// Returns (true, errorMessage) if the limit would be exceeded, (false, "") otherwise.
func dailyLimitExceeded(policy *model.ApprovalPolicy, amount string) (bool, string) {
	...
	if policy.DailyResetAt.Before(startOfDay) {
		currentSpent = "0" // new day — counter would be reset
	}
