...
func main() {
	...
	firstAppInstanceID := client.GetDefaultAppInstanceID()
	firstAppKeyName := schnorrResult.PublicKey.Name

	fmt.Printf("\n📋 Original App Instance ID: %s\n", firstAppInstanceID)
	...
	secondAppInstanceID := os.Getenv("SECOND_APP_INSTANCE_ID")
	if secondAppInstanceID == "" {
	...
	client.SetDefaultAppInstanceID(secondAppInstanceID)

	fmt.Printf("\n🔄 Switching to different App Instance ID: %s\n", secondAppInstanceID)
	...
	if err != nil {
		...
		fmt.Println("\n⚠️  ISSUE REPRODUCED!")
		fmt.Printf("   Key created by App Instance ID: %s\n", firstAppInstanceID)
		fmt.Printf("   Attempted to sign for App Instance ID: %s\n", secondAppInstanceID)
		fmt.Println("\n💡 Explanation:")
	...
	// ========================================================================
	thirdAppInstanceID := os.Getenv("THIRD_APP_INSTANCE_ID")
	if thirdAppInstanceID == "" {
		fmt.Println("⚠️  THIRD_APP_INSTANCE_ID not set, skipping third app test")
		// Restore original app ID
		client.SetDefaultAppInstanceID(firstAppInstanceID)
		fmt.Println("\n" + repeat("=", 70))
		fmt.Println("Cross-app signing test completed!")
		fmt.Println(repeat("=", 70))
		return
	}
	client.SetDefaultAppInstanceID(thirdAppInstanceID)

	fmt.Printf("\n🔄 Switching to third App Instance ID: %s\n", thirdAppInstanceID)
	fmt.Println("\n📝 Attempting to sign with the key from the first app...")
